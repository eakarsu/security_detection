...
-- Insert realistic security events
...
('malware_detection', 'critical', '192.168.1.45', NULL, 'jsmith', '/home/jsmith/Downloads/invoice.exe', 'Ransomware detected on endpoint - WannaCry variant', 
 ...
 0.98, '{"threat_type": "ransomware", "family": "wannacry", "confidence": 0.98, "recommended_action": "immediate_isolation"}', 'open', NOW() - INTERVAL '2 hours'),
...
('data_exfiltration', 'critical', '192.168.1.67', '45.142.214.219', 'mwilson', '/api/customer-data', 'Large data transfer to known malicious IP detected', 
 ...
 0.95, '{"anomaly_score": 0.95, "baseline_deviation": "500x normal", "data_classification": "sensitive"}', 'investigating', NOW() - INTERVAL '45 minutes'),
...
('privilege_escalation', 'critical', '192.168.1.23', NULL, 'temp_user', '/bin/sudo', 'Unauthorized privilege escalation attempt detected', 
 ...
 0.92, '{"attack_pattern": "credential_stuffing", "user_risk_score": 0.89, "account_age_days": 2}', 'open', NOW() - INTERVAL '1 hour'),
...
('suspicious_login', 'high', '103.224.182.251', '192.168.1.10', 'admin', '/admin/login', 'Admin login from suspicious IP address', 
 ...
 0.87, '{"risk_factors": ["tor_usage", "admin_account", "unusual_location"], "recommendation": "require_2fa"}', 'investigating', NOW() - INTERVAL '3 hours'),
...
('network_anomaly', 'high', '192.168.1.89', '8.8.8.8', 'system', 'DNS', 'Unusual DNS query pattern detected - possible DNS tunneling', 
 ...
 0.84, '{"technique": "dns_tunneling", "data_volume_mb": 15.7, "detection_confidence": 0.84}', 'open', NOW() - INTERVAL '30 minutes'),
...
('brute_force', 'high', '194.147.140.123', '192.168.1.5', 'multiple', '/ssh', 'SSH brute force attack detected', 
 ...
 0.91, '{"attack_type": "dictionary_attack", "targeted_accounts": ["root", "admin", "user"], "source_reputation": "malicious"}', 'blocked', NOW() - INTERVAL '4 hours'),
...
('failed_login', 'medium', '192.168.1.156', '192.168.1.10', 'bthomas', '/login', 'Multiple failed login attempts from internal user', 
 ...
 0.65, '{"user_behavior": "anomalous", "possible_causes": ["forgotten_password", "account_compromise"], "risk_level": "medium"}', 'open', NOW() - INTERVAL '6 hours'),
...
('policy_violation', 'medium', '192.168.1.78', '91.240.118.172', 'kjohnson', '/web-proxy', 'Access to blocked website category detected', 
 ...
 0.72, '{"violation_type": "web_filtering", "user_training_required": true, "repeat_offender": false}', 'resolved', NOW() - INTERVAL '8 hours'),
...
('file_integrity', 'medium', '192.168.1.34', NULL, 'system', '/etc/passwd', 'Critical system file modification detected', 
 ...
 0.68, '{"legitimate_change": "likely", "admin_approval": "pending", "rollback_available": true}', 'investigating', NOW() - INTERVAL '12 hours'),
...
('port_scan', 'low', '192.168.1.200', '192.168.1.0/24', 'security_scanner', 'network', 'Authorized vulnerability scan detected', 
 ...
 0.25, '{"authorized_scan": true, "scanner_authenticated": true, "compliance_scan": true}', 'resolved', NOW() - INTERVAL '1 day'),

('software_update', 'low', '192.168.1.99', 'update.microsoft.com', 'system', 'windows_update', 'System update installation completed', 
 '{"updates_installed": 15, "security_updates": 8, "reboot_required": true, "installation_time": "45 minutes"}', 
 0.15, '{"update_criticality": "important", "security_impact": "positive", "system_stability": "stable"}', 'resolved', NOW() - INTERVAL '2 days');
...
('total_events_today', 47, 'count', '{"period": "24h", "severity": "all"}', NOW()),
('critical_events_today', 3, 'count', '{"period": "24h", "severity": "critical"}', NOW()),
('high_events_today', 8, 'count', '{"period": "24h", "severity": "high"}', NOW()),
('medium_events_today', 21, 'count', '{"period": "24h", "severity": "medium"}', NOW()),
('low_events_today', 15, 'count', '{"period": "24h", "severity": "low"}', NOW()),
